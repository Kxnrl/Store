...
class Store
{
    ...
    public function __construct() {
        ...
        /*
        if(!file_exists(__DIR__ . "/database.lock")) {

            $fop = fopen(__DIR__ . "/dbImport.sql", "r") or die("Failed to open SQL query file.");
            $sql = fread($fop, filesize(__DIR__ . "/dbImport.sql"));
            fclose($fop);
            trim($sql);
            print_r("create SQL PROCEDURE: " . PHP_EOL . $sql);
            if(!$this->dbConnection->query($sql)) {
                $this->Destroy();
                throw new StoreException("Failed to create SQL PROCEDURE: " . $this->dbConnection->error);
            }
        }
        */
    }
...
class Player extends Store
{
    ...
    public function __construct($userId) {
        ...
        if(!is_numeric($userId) || $userId >= 76561100000000000) {
            ...
            try {
                ...
                $steam2Id = str_replace(array("STEAM_0:", "STEAM_1:"), "", $CSteamID->RenderSteam2());
    ...
    public function transfer($userId, $credits) {
        ...
        if($row['errCode'] != 0) {
            throw new StoreException("Failed to handle transfering -> errCode: " . $row['errCode'] . "  errMsgs: " . $row['errMsgs']);
        }
    ...
    public function sellcheck($unique_id) {
        ...
        if($_config['store']['selling']['poundage'] > 1.0) { $_config['store']['selling']['poundage'] = 1.0; }
        if($_config['store']['selling']['poundage'] < 0.0) { $_config['store']['selling']['poundage'] = 0.0; }
        ...
        return array('itemId' => $row['id'], 'unique' => $row['unique_id'], 'price' => $price, 'poundage' => ($row['price_of_purchase'] - $price));
    }
    ...
    private function hasItem($userId, $unique_id) {
        ...
        $has = ($result->num_rows > 0) ? true : false;
        $result->free();
    ...
    public function gifting($userId, $itemId) {
        ...
        return array('itemName' => $row['itemShortName'], 'itemUId' => $row['itemUniqueId']);
    }
}
