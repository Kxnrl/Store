...
public void OnMapStart()
{
    ...
    for (int x = 0; x < sizeof(g_szDefaultSkin); ++x)
        if (g_bSkinLoaded[x])
            AddFileToDownloadsTable(g_szDefaultSkin[x]);
    for (int x = 0; x < sizeof(g_szDefaultArms); ++x)
        if (g_bArmsLoaded[x])
            AddFileToDownloadsTable(g_szDefaultArms[x]);
}
...
{
    if (g_bArmsLoaded[team])
        strcopy(arms, armsLen, g_szDefaultArms[team]);
    if (g_bSkinLoaded[team])
    {
        strcopy(skin, skinLen, g_szDefaultSkin[team]);
        return true;
    }
