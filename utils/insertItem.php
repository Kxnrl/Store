    ...
    foreach($module_data as $parent_name => $parent_data)
    {
        foreach($parent_data as $data_1_key => $data_1_value)
        {
            if(array_key_exists('type', $data_1_value)){
                ...
                $escape['idle'] = mysqli_real_escape_string($database, isset($data['idle']) ? $data['idle'] : "ITEM_NO_IDLE");
                $escape['run'] = mysqli_real_escape_string($database, isset($data['run']) ? $data['run'] : "ITEM_NO_RUN");
                $escape['death'] = mysqli_real_escape_string($database, isset($data['death']) ? $data['death'] : "ITEM_NO_DEATH");
                $escape['seed'] = isset($data['seed']) ? $data['seed'] : 0;
                $escape['weart'] = isset($data['weart']) ? $data['weart'] : -1;
                $escape['paint'] = isset($data['paint']) ? $data['paint'] : 0;
                $escape['wearf'] = isset($data['wearf']) ? $data['wearf'] : 0.0;
                $sql = "INSERT IGNORE INTO store_item_child VALUES ((SELECT id FROM store_item_parent WHERE name = '".$escape['parent']."'), '".$escape['type']."', '".$escape['uid']."', ".$escape['buyable'].", ".$escape['giftable'].", ".$escape['only'].", '".$escape['auth']."', ".$escape['vip'].", '".$escape['name']."', '".$escape['lvls']."', '".$escape['desc']."', ".$escape['case'].", ".$escape['compose'].", '".$escape['1d']."', '".$escape['1m']."', '".$escape['pm']."', '".$escape['model']."', '".$escape['arms']."', '".$escape['team']."', '".$escape['sound']."', '".$escape['position']."', '".$escape['angles']."', '".$escape['attachment']."', '".$escape['slot']."', '".$escape['material']."', '".$escape['color']."', '".$escape['grenade']."', '".$escape['shortname']."', '".$escape['volume']."', '".$escape['cooldown']."', '".$escape['worldmodel']."', '".$escape['dropmodel']."', '".$escape['weapon']."', '".$escape['effect']."', '".$escape['brightness']."', '".$escape['distance']."', '".$escape['distancefade']."', '".$escape['tag']."', '".$escape['idle']."', '".$escape['run']."', '".$escape['death']."', '".$escape['seed']."', '".$escape['weart']."', '".$escape['paint']."', '".$escape['wearf']."')";
                $result = mysqli_query($database, $sql);
                if(mysqli_affected_rows($database) < 1){
                    echo "Insert Failed 1: ".mysqli_error($database)." -> ".$sql."\n";
                }
                ...
                foreach($data_1_value as $data_2_key => $data_2_value)
                {
                    if(array_key_exists('type', $data_2_value)){
                        ...
                        $sql = "INSERT IGNORE INTO store_item_child VALUES ((SELECT id FROM store_item_parent WHERE name = '".$escape['parent']."'), '".$escape['type']."', '".$escape['uid']."', ".$escape['buyable'].", ".$escape['giftable'].", ".$escape['only'].", '".$escape['auth']."', ".$escape['vip'].", '".$escape['name']."', '".$escape['lvls']."', '".$escape['desc']."', ".$escape['case'].", ".$escape['compose'].", '".$escape['1d']."', '".$escape['1m']."', '".$escape['pm']."', '".$escape['model']."', '".$escape['arms']."', '".$escape['team']."', '".$escape['sound']."', '".$escape['position']."', '".$escape['angles']."', '".$escape['attachment']."', '".$escape['slot']."', '".$escape['material']."', '".$escape['color']."', '".$escape['grenade']."', '".$escape['shortname']."', '".$escape['volume']."', '".$escape['cooldown']."', '".$escape['worldmodel']."', '".$escape['dropmodel']."', '".$escape['weapon']."', '".$escape['effect']."', '".$escape['brightness']."', '".$escape['distance']."', '".$escape['distancefade']."', '".$escape['tag']."', '".$escape['idle']."', '".$escape['run']."', '".$escape['death']."', '".$escape['seed']."', '".$escape['weart']."', '".$escape['paint']."', '".$escape['wearf']."')";;
                        if(mysqli_affected_rows($database) < 1){
                            echo "Insert Failed 2: ".mysqli_error($database)." -> ".$sql."\n";
                        }
                        if(isset($store[$data['type']][$data['name']])){
                            print_r($data['name']." is already exists!\n");
                            print_r($data);
                        ...
                        foreach($data_2_value as $data_3_key => $data_3_value)
                        {
                            if(array_key_exists('type', $data_3_value)){
                                ...
                                $result = mysqli_query($database, $sql);
                                if(mysqli_affected_rows($database) < 1){
                                    echo "Insert Failed 3: ".mysqli_error($database)." -> ".$sql."\n";
                                }
                                if(isset($store[$data['type']][$data['name']])){
                                    print_r($data['name']." is already exists!\n");
                                    print_r($data);
                                ...
                                foreach($data_3_value as $data_4_key => $data_4_value)
                                {
                                    if(array_key_exists('type', $data_4_value)){
                                        $data['name'] = $data_4_key;
                                        $data['module'] = $module_name;
                                        $data['parent'] = $parent_name." -> ".$data_1_key." -> ".$data_2_key." -> ".$data_3_key;
                                        foreach($data_4_value as $key => $value)
                                        {
                                            if(!is_array($value)){
                                                if($key == 'price'){
                                                    $data['永久'] = $value;
                                                ...
                                                print_r("can not load ".$data['name']." \n");
                                                print_r($data_4_value);
                                                print_r("\n");
                                            }
                                        }
                                        $items[$module_name][$parent_name][$data_1_key][$data_2_key][$data_3_key][$data['name']] = $data;
                                        $escape = array();
                                        $escape['parent'] = mysqli_real_escape_string($database, $data_2_key);
                                        $escape['type'] = mysqli_real_escape_string($database, $data['type']);
                                        $escape['uid'] = mysqli_real_escape_string($database, $data['uid']);
                                        ...
                                        $result = mysqli_query($database, $sql);
                                        if(mysqli_affected_rows($database) < 1){
                                            echo "Insert Failed 4: ".mysqli_error($database)." -> ".$sql."\n";
                                        }
                                        if(isset($store[$data['type']][$data['name']])){
                                            print_r($data['name']." is already exists!\n");
                                            print_r($data);
